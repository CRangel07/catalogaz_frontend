<template>
  <div class="group relative w-64 h-full cursor-pointer select-none">
    <div
      ...
      class="h-full w-full bg-slate-800/70 absolute z-100 cursor-not-allowed rounded-2xl flex items-center justify-center">
      <span class="text-xl italic font-semibold text-slate-300">No disponible</span>
    </div>
    ...
    <div
      ...
      <!-- Top accent bar -->
      <div class="h-1.5 w-full bg-linear-to-r from-azul to-naranja" />

      <div
        class="from-azul bg-linear-to-r to-naranja px-4 py-2 flex items-center justify-between"></div>
      ...
      <div
        ...
        :class="{
          'group-hover:scale-105': product.isActive,
        }"
      ...
      <div class="px-5 pb-5 pt-3 flex flex-col flex-1">
        <div class="flex-1">
          <h4 class="text-sm font-mono text-slate-400">#{{ product.code }}</h4>
          <h3 class="font-bold text-lg leading-tight text-azul">
            {{ product.name }}
          </h3>
          ...
          <p v-if="product.description" class="text-slate-500 text-sm mt-2 line-clamp-3 min-h-14">
            {{ product.description }}
          </p>
        </div>
        ...
        <div>
          ...
          <div class="mt-3 min-h-14">
            <p class="text-2xl font-extrabold tracking-tight text-azul">
              {{ formatMXNNoCents(Number(product.price.toFixed(2).split('.')[0])) }}
              <span class="text-base font-semibold text-naranja">
                .{{ String(product.price.toFixed(2)).split('.')[1] }}
              </span>
            </p>

            <!-- Subtotal (espacio reservado siempre) -->
            <p class="text-xs text-blue-500 font-medium mt-0.5 h-4">
